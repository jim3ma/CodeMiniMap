...
class Scrollbar(private val editor: Editor, private val scrollstate: ScrollState) : JPanel(), Disposable {
    ...
    private fun isInReizeGutter(x: Int): Boolean {
        if (config.widthLocked) {
            return false
        }
        return if (config.rightAligned)
            x in 0..7
        else
    ...
    inner class MouseHandler : MouseAdapter() {
        ...
        override fun mouseDragged(e: MouseEvent?) {
            if (resizing) {
                val newWidth =
                    widthStart + if (config.rightAligned) resizeStart - e!!.xOnScreen else e!!.xOnScreen - resizeStart
        ...
        override fun mouseMoved(e: MouseEvent?) {
            cursor = if (isInReizeGutter(e!!.x)) {
                if (config.rightAligned) Cursor(Cursor.W_RESIZE_CURSOR) else Cursor(Cursor.E_RESIZE_CURSOR)
            } else {
                defaultCursor
            }
